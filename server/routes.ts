...
export async function registerRoutes(
  ...
  // POST /api/margin-recommendations - Generate AI recommendations for margin diagnostic
  app.post("/api/margin-recommendations", async (req, res) => {
    try {
      const { inputs, result } = req.body;
      
      if (!inputs || !result) {
        return res.status(400).json({ error: "Missing inputs or result" });
      }
      ...
      const segmentoLabel = inputs.segmento === 'musculacao' ? 'Máquinas de Musculação' : 'Ventiladores HVLS';
      
      const indicadoresCriticos = [];
      if (result.margemBruta.status !== 'saudavel') {
        indicadoresCriticos.push(`Margem Bruta: ${result.margemBruta.valorFormatado} (${result.margemBruta.status === 'critico' ? 'CRÍTICO' : 'ATENÇÃO'}) - Ideal: ${result.margemBruta.benchmarkFormatado}`);
      }
      if (result.margemEbitda.status !== 'saudavel') {
        indicadoresCriticos.push(`Margem EBITDA: ${result.margemEbitda.valorFormatado} (${result.margemEbitda.status === 'critico' ? 'CRÍTICO' : 'ATENÇÃO'}) - Ideal: ${result.margemEbitda.benchmarkFormatado}`);
      }
      if (result.markup.status !== 'saudavel') {
        indicadoresCriticos.push(`Markup: ${result.markup.valorFormatado} (${result.markup.status === 'critico' ? 'CRÍTICO' : 'ATENÇÃO'}) - Ideal: ${result.markup.benchmarkFormatado}`);
      }
      if (result.pontoEquilibrio.status !== 'saudavel') {
        indicadoresCriticos.push(`Ponto de Equilíbrio: ${result.pontoEquilibrio.valorFormatado} (${result.pontoEquilibrio.status === 'critico' ? 'CRÍTICO' : 'ATENÇÃO'})`);
      }

      const prompt = `Você é um consultor industrial experiente. Analise este diagnóstico financeiro e forneça recomendações práticas.

DADOS DA EMPRESA:
- Segmento: ${segmentoLabel}
- Receita: R$ ${result.receita.toLocaleString('pt-BR')}
- CMV: R$ ${result.cmv.toLocaleString('pt-BR')}
- Custos Fixos: R$ ${result.custosFixos.toLocaleString('pt-BR')}
- Custos Variáveis: R$ ${result.custosVariaveis.toLocaleString('pt-BR')}

INDICADORES COM PROBLEMAS:
${indicadoresCriticos.length > 0 ? indicadoresCriticos.join('\n') : 'Todos os indicadores estão saudáveis!'}

${indicadoresCriticos.length > 0 ? `Para CADA indicador problemático, forneça:

🔴 [NOME DO INDICADOR]
📊 Diagnóstico: Por que provavelmente está assim (1 frase)
✅ Ação Rápida: O que fazer HOJE para melhorar (1 frase)
🔧 Ação Estrutural: Mudança de médio prazo (1 frase)

Use linguagem simples de empresário, não contábil. Seja direto e prático.
Máximo 3-4 linhas por indicador.` : 'Parabenize o empresário pelos bons resultados e sugira como manter.'}`;
